...
FLYWAY_IMAGE="${FLYWAY_IMAGE:-redgate/flyway:latest}"
...
assert_sql "select count(*) from flyway_schema_history where success = true;" "2"
...
run_flyway "${ROOT_DIR}/migrations" baseline -baselineVersion=1 -baselineDescription=existing-schema
run_flyway "${ROOT_DIR}/migrations" migrate
run_flyway "${ROOT_DIR}/migrations" validate
...
assert_sql "select count(*) from flyway_schema_history where version = '2' and success = true;" "1"
